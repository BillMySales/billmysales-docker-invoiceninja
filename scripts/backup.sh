#!/bin/bash
# Backups of the Invoice Ninja database and its storage (logos, documents,
# generated files). The APP_KEY (in .env) decrypts stored secrets: keep it.
#
#   backup.sh            # loop: back up now, then every BACKUP_INTERVAL_HOURS
#   backup.sh now        # one backup
#   backup.sh list       # list backups
#   backup.sh restore <timestamp>   # restore database and storage
...
# (storage/), deleted after BACKUP_KEEP_DAYS days.
set -euo pipefail
...
backup() {
    ...
    echo "==> Backup ${ts}"
    tmp="${BACKUP_DIR}/.${ts}"
    mysqldump --defaults-extra-file="${DB_CNF}" --single-transaction --quick --no-tablespaces \
        --routines --triggers --events "${DB_NAME}" | gzip > "${tmp}-db.sql.gz"
    tar -C "${DATA_DIR}" -czf "${tmp}-files.tar.gz" storage
...
restore() {
    ...
    mysql --defaults-extra-file="${DB_CNF}" --batch --skip-column-names -e \
        "SELECT CONCAT('DROP TABLE IF EXISTS \`', table_name, '\`;') FROM information_schema.tables WHERE table_schema = '${DB_NAME}'" |
        { echo 'SET FOREIGN_KEY_CHECKS = 0;'; cat; } | mysql --defaults-extra-file="${DB_CNF}" "${DB_NAME}"
    gunzip -c "${db}" | mysql --defaults-extra-file="${DB_CNF}" "${DB_NAME}"
    echo "==> Restoring storage from ${files}"
    find "${DATA_DIR}/storage" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
